...
class Timer(QtGui.QWidget):
    ...
    def settt(self):
        self.fade = 1
        self.number = float(self.checkInputValue(self.edit.text()))
    ...
    def checkInputValue(self, val):
        dotCount = val.count(".")
        if dotCount <= 1:
            valCopy = val[:]
            valCopy.replace(".", "")
            if str(valCopy).isdigit()==False:
                self.edit.setText("25")
                return 25
            else:
                return val
        else:
